...
struct proc * scheduler_q1(void)
{
  ...
  // a = rand;
	...
	for(choice = ptable.proc; choice < &ptable.proc[NPROC]; choice++)
  {
	    ...
	    if(choice->pid == 1)
	  	...
	  	if(choice->queue_number == 1)
	  		tickets += choice->tickets;
	}

	if(tickets)
		tickets = rand % tickets;
...
struct proc * scheduler_q2(void)
{
	...
	for (current = ptable.proc; current < &ptable.proc[NPROC]; ++current)
  {
	...
	if (choice != 0)
		choice->execution_cycle_number += 0.1;
...
struct proc * scheduler_q3(void){
  struct proc* current;
	...
	for (current = ptable.proc; current < &ptable.proc[NPROC]; ++current){
		...
		if (current -> queue_number == 3){
			if (current->priority < minimum_priority){
...
void set_queue(int pid, int queue)
{
  acquire(&ptable.lock);
	...
	if(!p)
  {
    cprintf("set_queue: Process not found!\n");
    release(&ptable.lock);
		return;
  }

  cprintf("Last queue number: %d\n", p->queue_number);
	p->queue_number = queue;
  release(&ptable.lock);
}

void set_tickets(int pid, int t)
{
  acquire(&ptable.lock);
	struct proc *p = find_proc(pid);
	...
	if(!p)
	{
    cprintf("set_tickets: Process not found!\n");
    release(&ptable.lock);
		return;
  }

  cprintf("Last tickets: %d\n", p->tickets);
	p->tickets = t;
  release(&ptable.lock);
}
...
void set_priority(int pid, int priority)
{
  ...
  if(!p)
	{
    cprintf("set_priority: Process not found!\n");
    release(&ptable.lock);
		return;
  }

  cprintf("Last priority: %d\n", p->priority);
	p->priority = priority;
  release(&ptable.lock);
}

char* get_state_type(enum procstate state)
{
	switch (state)
	{
	case UNUSED:
...
void show_all_info(void) {
	struct proc* p;
	acquire(&ptable.lock);
	cprintf("name\t\tpid\tstate\t\tqueue\t\tlottery\t\tpriority\tcreation time\texecuted cycle\n");
	cprintf("------------------------------------------------------------------------------------------------------------"
	"----------\n");
	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
	{
		if(p->state != UNUSED)
		{
			const char* state = get_state_type(p->state);
			
			if (strlen(p->name) >= 8)
				cprintf("%s\t%d\t%s\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
					p->name, p->pid, state, p->queue_number, p->tickets, (int)p->priority, (int)p->arrival_time, (int)p->execution_cycle_number);
			else
				cprintf("%s\t\t%d\t%s\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",
					p->name, p->pid, state, p->queue_number, p->tickets, (int)p->priority, (int)p->arrival_time, (int)p->execution_cycle_number);
		}
